...
function decorateChildren(
  ...
  for (; slot < length; slot++) {
    ...
    active.length = kept;
    if (kept === 0) {
      // No range reaches this block. With ranges still to come, jump to the
      // first block the next one can reach instead of walking the gap.
      if (order === undefined && cursor < ranges.length) {
