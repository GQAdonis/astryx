...
function replaceTextNode<Node extends MarkdownExtensionNode>(
  ...
  while ((match = pattern.exec(value)) != null) {
    ...
    // introduce. Clone them away from the caller, then validate them here
    // under the same rules Core applies to newly-introduced nodes, so the
    // tree this transform returns needs no further checking.
    const cloned: Phrasing[] = [];
    for (const replacementNode of replacements) {
      cloned.push(cloneReplacement(replacementNode));
    }
    validateMarkdownHelperNodes(cloned, ownership);
    output.push(...cloned);
    cursor = resolvedEnd;
    pattern.lastIndex =
      resolvedEnd === start
